...
function renderAnalContent(){
  ...
  // Build graded + missing arrays — keep track of WHO is missing
  const stuWithGrade=[];const stuMissing=[];
  stu.forEach(s=>{
    let g=null;
    if(q===0){
      const vq=[1,2,3,4].map(qq=>calcQ(s.name,qq).quarterly).filter(v=>v!==null);
      g=vq.length?Math.round(vq.reduce((a,b)=>a+b,0)/vq.length):null;
    }else{
      g=calcQ(s.name,q).quarterly;
    }
  ...
  if(!grades.length){
    area.innerHTML=`
    <div class="card">
      <div class="ch"><div class="ct" style="color:var(--red)">⚠ No grades entered yet for ${q===0?'any quarter':'Quarter '+q}</div></div>
      <div style="font-size:.88rem;color:var(--tx2);margin-bottom:1rem">All ${stu.length} students have missing grades for this quarter.</div>
      <div style="font-size:.82rem;font-weight:600;color:var(--tx3);margin-bottom:.6rem">ALL STUDENTS WITH MISSING GRADES</div>
      <div style="display:flex;flex-wrap:wrap;gap:.4rem">
        ${stu.map(s=>`<span style="background:var(--red2);color:var(--red);padding:.2rem .6rem;border-radius:99px;font-size:.78rem;font-weight:500">${escH(s.name)}</span>`).join('')}
      </div>
    </div>`;
    return;
  }
  ...
  const hi=Math.max(...grades);const lo=Math.min(...grades);
  const pass=grades.filter(g=>g>=75).length;const fail=grades.filter(g=>g<75).length;
  const miss=stuMissing.length;
  const bands=[
  ...
  ].map(b=>({...b,cnt:grades.filter(g=>g>=b.mn&&g<=b.mx).length,students:stuWithGrade.filter(s=>s.grade>=b.mn&&s.grade<=b.mx)}));
  const mx=Math.max(...bands.map(b=>b.cnt),1);

  area.innerHTML=`
  <div style="font-size:.78rem;color:var(--tx3);margin-bottom:.75rem;padding:.5rem .75rem;background:var(--surf2);border-radius:var(--rs);border:1px solid var(--bdr)">${escH(cd.school.subject||'—')} · ${escH(APP.ac.grade)} ${escH(APP.ac.section)} · ${q===0?'All Quarters — Final Grade':'Quarter '+q}</div>

  <!-- Stat Cards -->
  <div class="ag4" style="margin-bottom:1rem">
  ...
  <!-- Performance Bands + Pass/Fail side by side -->
  <div class="g2" style="margin-bottom:1rem">
    <!-- Performance Bands -->
    <div class="card">
      <div class="ch"><div class="ct">📊 Performance Bands</div><div class="cs">Click a band to see student list</div></div>
      ${bands.map((b,i)=>`
        <div class="brow" style="cursor:${b.cnt>0?'pointer':'default'}" onclick="${b.cnt>0?`showBandStudents(${i})`:''}">
          <div class="bla">${b.l}</div>
          <div class="bbw"><div class="bb" style="width:${Math.round((b.cnt/mx)*100)}%;background:${b.c}"></div></div>
          <div class="bcnt">${b.cnt}</div>
        </div>`).join('')}
    ...
    <!-- Pass / Fail -->
    <div class="card">
      <div class="ch"><div class="ct">✅ Pass / Fail Summary</div></div>
      ${[{l:'Passing (≥75)',v:pass,c:'var(--green)',bg:'var(--green2)'},{l:'Below 75',v:fail,c:'var(--red)',bg:'var(--red2)'}].map(r=>`
      <div style="margin-bottom:1rem">
        <div class="fl" style="margin-bottom:.4rem;justify-content:space-between">
          <span style="font-size:.85rem;color:${r.c};font-weight:600">${r.l}</span>
          <span style="font-size:.85rem;font-family:'DM Mono',monospace;font-weight:700">${r.v} / ${grades.length}</span>
        </div>
        <div style="background:var(--bg);border-radius:99px;height:9px;overflow:hidden">
          <div style="height:100%;border-radius:99px;background:${r.c};width:${grades.length?Math.round((r.v/grades.length)*100):0}%;transition:width .5s"></div>
        </div>
        <div style="font-size:.75rem;color:var(--tx3);margin-top:.25rem">${grades.length?Math.round((r.v/grades.length)*100):0}% of graded students</div>
      </div>`).join('')}
      <div class="g2" style="margin-top:.75rem">
        <div style="text-align:center;padding:.75rem;background:var(--blue2);border-radius:var(--rs)">
          <div style="font-size:.75rem;color:var(--blue);margin-bottom:.2rem;font-weight:600">Top Performers</div>
          <div style="font-size:1.5rem;font-weight:700;font-family:'DM Mono',monospace;color:var(--blue)">${grades.filter(g=>g>=90).length}</div>
          <div style="font-size:.72rem;color:var(--blue);opacity:.7">Grade 90+</div>
        </div>
        <div style="text-align:center;padding:.75rem;background:${miss>0?'var(--red2)':'var(--green2)'};border-radius:var(--rs)">
          <div style="font-size:.75rem;color:${miss>0?'var(--red)':'var(--green)'};margin-bottom:.2rem;font-weight:600">Missing Grades</div>
          <div style="font-size:1.5rem;font-weight:700;font-family:'DM Mono',monospace;color:${miss>0?'var(--red)':'var(--green)'}"> ${miss}</div>
          <div style="font-size:.72rem;color:${miss>0?'var(--red)':'var(--green)'};opacity:.8">${miss===0?'All graded ✓':'students'}</div>
        </div>
      </div>
    </div>
  </div>

  <!-- Missing Grades Section — always visible when there are missing -->
  ${miss>0?`
  <div class="card" style="border:2px solid var(--red);margin-bottom:1rem">
    <div class="ch" style="margin-bottom:.75rem;padding-bottom:.6rem">
      <div>
        <div class="ct" style="color:var(--red)">⚠ Students with Missing Grades (${miss})</div>
        <div class="cs">${q===0?'These students have no final grade computed — at least one quarter is missing':'These students have no Quarterly Grade for Quarter '+q+' — HPS may not be set or no scores entered'}</div>
      </div>
      <span style="background:var(--red2);color:var(--red);padding:.3rem .8rem;border-radius:99px;font-size:.82rem;font-weight:700">${miss} student${miss>1?'s':''}</span>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:.5rem">
      ${stuMissing.map((s,i)=>`
        <div style="display:flex;align-items:center;gap:.6rem;padding:.5rem .75rem;background:var(--red2);border:1px solid rgba(220,38,38,.2);border-radius:var(--rs)">
          <div style="width:22px;height:22px;border-radius:50%;background:var(--red);color:#fff;font-size:.7rem;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0">${i+1}</div>
          <div style="flex:1">
            <div style="font-size:.85rem;font-weight:600;color:var(--tx)">${escH(s.name)}</div>
            <div style="font-size:.72rem;color:var(--tx3)">${s.gender==='male'?'👦 Male':'👧 Female'}</div>
          </div>
          <button class="btn bsm" style="padding:.3rem .6rem;font-size:.72rem;background:var(--red);color:#fff;border:none" onclick="goToMissingStudent('${esc(s.name)}',${q===0?1:q})">Go →</button>
        </div>`).join('')}
    </div>
    <div style="margin-top:.75rem;padding:.6rem .75rem;background:var(--amber2);border-radius:var(--rs);font-size:.82rem;color:var(--amber);border-left:3px solid var(--amber)">
      💡 Tip: Click <strong>Go →</strong> next to any student to jump directly to their row in Grade Entry.
    </div>
  </div>`:'<div class="card" style="border:2px solid var(--green);text-align:center;padding:1.2rem"><div style="font-size:1.5rem;margin-bottom:.4rem">✅</div><div style="font-size:.95rem;font-weight:600;color:var(--green)">All students have grades for ${q===0?'all quarters':'Quarter '+q}!</div></div>'}

  <!-- Student Grade List (collapsible) -->
  <div class="card">
    <div class="ch" style="cursor:pointer" onclick="toggleAnalStudentList()">
      <div><div class="ct">👤 Full Student Grade List</div><div class="cs">All ${grades.length} graded students</div></div>
      <span id="analListToggle" style="font-size:.82rem;color:var(--blue);font-weight:600">▼ Show</span>
    </div>
    <div id="analStudentList" style="display:none">
      <div style="overflow-x:auto"><table class="stbl">
        ...
        <tbody>
          ${(()=>{
            ...
            const sorted=[...stuWithGrade].sort((a,b)=>b.grade-a.grade);
            sorted.forEach((s,i)=>{
              const band=bands.find(b=>s.grade>=b.mn&&s.grade<=b.mx);
              rows.push(`<tr>
                <td style="text-align:center;font-family:'DM Mono',monospace;font-size:.78rem;color:var(--tx3)">${i+1}</td>
                <td style="font-weight:500;font-size:.85rem">${escH(s.name)} <span style="font-size:.72rem;color:var(--tx4)">${s.gender==='male'?'M':'F'}</span></td>
                <td style="text-align:center;font-family:'DM Mono',monospace;font-size:.9rem;font-weight:700;color:${s.grade>=75?'var(--green)':'var(--red)'}">${s.grade}</td>
                <td style="text-align:center"><span style="font-size:.72rem;font-weight:600;padding:.15rem .5rem;border-radius:99px;background:${band?band.c+'22':''};color:${band?band.c:''}">${band?band.l:'—'}</span></td>
                <td style="text-align:center"><span class="${s.grade>=75?'bpass':'bfail'}">${s.grade>=75?'PASSED':'FAILED'}</span></td>
              </tr>`);
            });
            return rows.join('');
          })()}
  ...
  // Store band data for click handler
  window._analyticsBands=bands;
  window._analyticsQ=q;
}

function toggleAnalStudentList(){
  const el=document.getElementById('analStudentList');
  const tog=document.getElementById('analListToggle');
  if(!el||!tog)return;
...
function showBandStudents(bandIdx){
  if(!window._analyticsBands)return;
  const band=window._analyticsBands[bandIdx];
  if(!band||!band.students.length)return;
  const q=window._analyticsQ;
  const html=`
    <div style="margin-bottom:.75rem;font-size:.88rem;color:var(--tx2)">
      ${band.students.length} student${band.students.length>1?'s':''} in the <strong>${band.l}</strong> band for ${q===0?'Final Grade':'Quarter '+q}
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:.5rem">
      ${band.students.map((s,i)=>`
        <div style="display:flex;align-items:center;gap:.6rem;padding:.5rem .75rem;background:${band.c}22;border:1px solid ${band.c}55;border-radius:var(--rs)">
          <div style="width:36px;height:36px;border-radius:50%;background:${band.c};color:#fff;font-size:.88rem;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0">${s.grade}</div>
          <div>
            <div style="font-size:.88rem;font-weight:600;color:var(--tx)">${escH(s.name)}</div>
            <div style="font-size:.72rem;color:var(--tx3)">${s.gender==='male'?'👦 Male':'👧 Female'}</div>
          </div>
        </div>`).join('')}
    </div>`;
  const cd=getCD();
  openModal(`${band.l} Band — ${band.students.length} Student${band.students.length>1?'s':''}`, `${escH(cd.school.subject||'')} · ${q===0?'Final Grade':'Quarter '+q}`, html);
}

function goToMissingStudent(name,q){
  APP.gq=q;APP.hlStudent=name;
  showRB('gradeentry');showPage('recordbook');
  toast('📍 Jumping to '+name);
}
...
function renderGSStudDetail(){
  ...
  if(!name){area.innerHTML='<div class="ws"><h2>Select a student</h2></div>';return}
  const cd=getCD();const h=cd.hps[q];
  const g=(cd.grades[q]&&cd.grades[q][name])||{};
  const ww=g.ww||Array(10).fill(null);const pt=g.pt||Array(10).fill(null);const qa=g.qa!==undefined?g.qa:null;
  const c=calcQ(name,q);
  ...
  <div class="card">
    <div class="ch">
      <div><div class="ct">${escH(name)}</div><div class="cs">${APP.ac.grade} ${APP.ac.section} · Q${q} Detail</div></div>
      <div class="fl">
        <div style="text-align:center;padding:8px 14px;background:${c.quarterly?(c.quarterly>=75?'var(--green2)':' var(--red2)'):'var(--surf2)'};border-radius:var(--r);border:1px solid var(--bdr)">
          <div style="font-size:18px;font-weight:700;font-family:'DM Mono',monospace;color:${c.quarterly?(c.quarterly>=75?'var(--green)':' var(--red)'):'var(--tx3)'}">Q${q}: ${c.quarterly||'—'}</div>
        </div>
        <div style="text-align:center;padding:8px 14px;background:${final?(final>=75?'var(--blue2)':' var(--red2)'):'var(--surf2)'};border-radius:var(--r);border:1px solid 2px solid var(--blue)">
          <div style="font-size:18px;font-weight:700;font-family:'DM Mono',monospace;color:${final?(final>=75?'var(--blue)':' var(--red)'):'var(--tx3)'}">Final: ${final||'—'}</div>
          <div style="font-size:10px;color:var(--tx3)">${final?(final>=75?'PASSED':'FAILED'):''}</div>
        </div>
      </div>
    </div>
    <div class="fl" style="margin-bottom:14px;gap:7px">
      ${allQ.map(a=>`<div style="flex:1;min-width:65px;text-align:center;padding:7px;border:1px solid ${a.q===q?'var(--blue)':' var(--bdr)'};border-radius:var(--rs);background:${a.q===q?'var(--blue2)':' var(--surf2)'}">
        <div style="font-size:9px;color:var(--tx3);margin-bottom:2px">Q${a.q}</div>
        <div style="font-size:13px;font-weight:600;font-family:'DM Mono',monospace;color:${a.q===q?'var(--blue)':(a.c.quarterly?(a.c.quarterly>=75?'var(--green)':' var(--red)'):'var(--tx3)')}"> ${a.c.quarterly||'—'}</div>
      </div>`).join('')}
      <div style="flex:1;min-width:65px;text-align:center;padding:7px;border:2px solid var(--blue);border-radius:var(--rs);background:var(--blue2)">
        <div style="font-size:9px;color:var(--blue);margin-bottom:2px;font-weight:600">FINAL</div>
        <div style="font-size:14px;font-weight:700;font-family:'DM Mono',monospace;color:var(--blue)">${final||'—'}</div>
      </div>
    </div>
    <div style="margin-bottom:12px">
      <div class="fl" style="margin-bottom:7px"><span class="tag tb">Written Works (30%)</span><span style="font-size:10px;color:var(--tx2)">Total: ${c.wwT!==null?c.wwT:'—'} · PS: ${c.wwPS!==null?c.wwPS:'—'}% · WS: ${c.wwWS!==null?c.wwWS:'—'}</span></div>
      <div style="display:flex;flex-wrap:wrap;gap:4px">
        ${Array.from({length:wwC},(_,i)=>`<div style="text-align:center;padding:5px 3px;border:1px solid var(--bdr);border-radius:var(--rs);min-width:50px;background:var(--surf)">
          <div style="font-size:9px;color:var(--tx3);margin-bottom:2px">WW${i+1}${h.ww[i]?'<br>/'+h.ww[i]:''}</div>
          <div style="font-size:12px;font-weight:600;font-family:'DM Mono',monospace">${ww[i]!==null&&ww[i]!==undefined?ww[i]:'—'}</div>
        </div>`).join('')}
      </div>
    </div>
    <div style="margin-bottom:12px">
      <div class="fl" style="margin-bottom:7px"><span class="tag ta">Performance Tasks (50%)</span><span style="font-size:10px;color:var(--tx2)">Total: ${c.ptT!==null?c.ptT:'—'} · PS: ${c.ptPS!==null?c.ptPS:'—'}% · WS: ${c.ptWS!==null?c.ptWS:'—'}</span></div>
      <div style="display:flex;flex-wrap:wrap;gap:4px">
        ${Array.from({length:ptC},(_,i)=>`<div style="text-align:center;padding:5px 3px;border:1px solid var(--bdr);border-radius:var(--rs);min-width:50px;background:var(--surf)">
          <div style="font-size:9px;color:var(--tx3);margin-bottom:2px">PT${i+1}${h.pt[i]?'<br>/'+h.pt[i]:''}</div>
          <div style="font-size:12px;font-weight:600;font-family:'DM Mono',monospace">${pt[i]!==null&&pt[i]!==undefined?pt[i]:'—'}</div>
        </div>`).join('')}
      </div>
    </div>
    ${h.qa?`<div style="margin-bottom:12px">
      <div class="fl" style="margin-bottom:7px"><span class="tag tg">QA (20%)</span><span style="font-size:10px;color:var(--tx2)">Score: ${qa!==null?qa:'—'} / ${h.qa} · PS: ${c.qaPS!==null?c.qaPS:'—'}% · WS: ${c.qaWS!==null?c.qaWS:'—'}</span></div>
    </div>`:''}
    <div style="padding:10px 14px;background:var(--surf2);border-radius:var(--r);border:1px solid var(--bdr);display:flex;gap:16px;flex-wrap:wrap">
      ${[['WW WS',c.wwWS],['PT WS',c.ptWS],['QA WS',c.qaWS],['Initial',c.initial]].map(([l,v])=>`<div><div style="font-size:9px;color:var(--tx3)">${l}</div><div style="font-size:12px;font-weight:600;font-family:'DM Mono',monospace">${v!==null?v:'—'}</div></div>`).join('')}
      <div><div style="font-size:9px;color:var(--tx3)">Q${q} Grade</div><div style="font-size:16px;font-weight:700;font-family:'DM Mono',monospace;color:${c.quarterly?(c.quarterly>=75?'var(--green)':' var(--red)'):'var(--tx3)'}">${c.quarterly||'—'}</div></div>
    </div>
  </div>`;
}
...
function renderGSLOA(){
  ...
  function pct(n,t){return t?r2((n/t)*100):0}
  const finals=stu.map(s=>{const vq=[1,2,3,4].map(q=>calcQ(s.name,q).quarterly).filter(v=>v!==null);return vq.length?Math.round(vq.reduce((a,b)=>a+b,0)/vq.length):null;});
  const fb=descB(finals);
  const pass=finals.filter(v=>v!==null&&v>=75).length;
  const fail=finals.filter(v=>v!==null&&v<75).length;
  const wg=finals.filter(v=>v!==null).length;
  const avg=wg?r2(finals.filter(v=>v!==null).reduce((a,b)=>a+b,0)/wg):0;
  const qSum=[1,2,3,4].map(q=>{const qg=stu.map(s=>calcQ(s.name,q).quarterly);const vld=qg.filter(v=>v!==null);const qb=descB(qg);return{q,avg:vld.length?r2(vld.reduce((a,b)=>a+b,0)/vld.length):null,pass:vld.filter(v=>v>=75).length,fail:vld.filter(v=>v<75).length,total:vld.length,b:qb};});
  ...
  el.innerHTML=`
  <div style="font-size:11px;color:var(--tx2);padding:8px 12px;background:var(--surf2);border:1px solid var(--bdr);border-radius:var(--r);margin-bottom:14px;font-weight:500">
    📊 LOA Overview — ${escH(cd.school.subject||'Subject')} · ${escH(section)} · Teacher: ${escH(cd.school.teacher||'—')} · SY ${escH(cd.school.year||'—')}
  </div>
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:14px">
    <div class="stat-c"><div class="stat-n" style="color:var(--blue)">${avg}</div><div class="stat-l">Overall Average</div></div>
    <div class="stat-c"><div class="stat-n" style="color:var(--green)">${pass}</div><div class="stat-l">Passing (≥75)</div></div>
    <div class="stat-c"><div class="stat-n" style="color:var(--red)">${fail}</div><div class="stat-l">Below 75</div></div>
    <div class="stat-c"><div class="stat-n" style="color:var(--tx2)">${wg}/${N}</div><div class="stat-l">With Final Grade</div></div>
  </div>
  <div class="card" style="margin-bottom:14px">
    <div class="ch"><div class="ct">Final Grade Distribution (All Quarters)</div><div class="cs">Descriptor breakdown by achievement level</div></div>
    <div style="overflow-x:auto"><table class="loa-full-table">
      ...
      <tbody>
        ...
        <tr class="total-row"><td class="sec-col">Total</td><td>${N}</td>${bDefs.map(b=>`<td>${fb[b.k]||0}</td><td>${pct(fb[b.k]||0,N)}%</td>`).join('')}</tr>
      </tbody>
    </table></div>
  </div>
  <div class="g2">
...
function renderGSStudent(){
  ...
  el.innerHTML=`
  <div class="card" style="margin-bottom:12px">
    <div class="ch"><div class="ct">Student Detail</div><div class="cs">Select quarter and student</div></div>
    <div class="fl">
      <div><div class="lbl">Quarter</div><select class="inp" id="gssdQ" style="width:110px" onchange="renderGSSD()"><option value="1">Q1</option><option value="2">Q2</option><option value="3">Q3</option><option value="4">Q4</option></select></div>
      ...
      <button class="btn bp bsm" onclick="goToStudRow()">📍 Go to Row</button>
      <button class="btn br bsm" onclick="pdfGSSD()">📄 PDF</button>
    </div>
  </div>
  <div id="gssdArea"><div class="ws"><h2>Select a student</h2></div></div>`;
}

function renderGSSD(){
  const rawVal=document.getElementById('gssdName')?.value;
  const name=rawVal?rawVal.replace(/^\[.\] /,''):'';
  const q=parseInt(document.getElementById('gssdQ')?.value||'1');
  ...
  area.innerHTML=`<div class="card">
    <div class="ch">
      <div><div class="ct">${escH(name)}</div><div class="cs">${APP.ac.grade} ${APP.ac.section} · Q${q}</div></div>
      <div class="fl">
        <div style="text-align:center;padding:8px 14px;background:${c.quarterly?(c.quarterly>=75?'var(--green2)':'var(--red2)'):'var(--surf2)'};border-radius:var(--r);border:1px solid var(--bdr)">
          <div style="font-size:18px;font-weight:700;font-family:'DM Mono',monospace;color:${c.quarterly?(c.quarterly>=75?'var(--green)':'var(--red)'):'var(--tx3)'}">Q${q}: ${c.quarterly||'—'}</div>
        </div>
        <div style="text-align:center;padding:8px 14px;background:${final?(final>=75?'var(--blue2)':'var(--red2)'):'var(--surf2)'};border-radius:var(--r);border:1px solid var(--bdr)">
          <div style="font-size:18px;font-weight:700;font-family:'DM Mono',monospace;color:${final?(final>=75?'var(--blue)':'var(--red)'):'var(--tx3)'}">Final: ${final||'—'}</div>
          <div style="font-size:10px;color:var(--tx3)">${final?(final>=75?'PASSED':'FAILED'):''}</div>
        </div>
      </div>
    </div>
    <div class="fl" style="margin-bottom:12px;gap:6px">
      ${allQ.map(a=>`<div style="flex:1;text-align:center;padding:6px;border:1px solid ${a.q===q?'var(--blue)':'var(--bdr)'};border-radius:var(--rs);background:${a.q===q?'var(--blue2)':'var(--surf2)'}"><div style="font-size:9px;color:var(--tx3)">Q${a.q}</div><div style="font-size:13px;font-weight:600;font-family:'DM Mono',monospace;color:${a.q===q?'var(--blue)':(a.c.quarterly?(a.c.quarterly>=75?'var(--green)':'var(--red)'):'var(--tx3)')}">${a.c.quarterly||'—'}</div></div>`).join('')}
      <div style="flex:1;text-align:center;padding:6px;border:2px solid var(--blue);border-radius:var(--rs);background:var(--blue2)"><div style="font-size:9px;color:var(--blue);font-weight:600">FINAL</div><div style="font-size:14px;font-weight:700;font-family:'DM Mono',monospace;color:var(--blue)">${final||'—'}</div></div>
    </div>
    <div style="margin-bottom:10px"><div class="fl" style="margin-bottom:6px"><span class="tag tb">Written Works (30%)</span><span style="font-size:10px;color:var(--tx2)">Total: ${c.wwT!==null?c.wwT:'—'} · PS: ${c.wwPS!==null?c.wwPS:'—'}% · WS: ${c.wwWS!==null?c.wwWS:'—'}</span></div>
      <div style="display:flex;flex-wrap:wrap;gap:4px">${Array.from({length:wwC},(_,i)=>`<div style="text-align:center;padding:4px 3px;border:1px solid var(--bdr);border-radius:var(--rss);min-width:46px;background:var(--surf)"><div style="font-size:9px;color:var(--tx3)">WW${i+1}${h.ww[i]?'<br>/'+h.ww[i]:''}</div><div style="font-size:11px;font-weight:600;font-family:'DM Mono',monospace">${ww[i]!==null&&ww[i]!==undefined?ww[i]:'—'}</div></div>`).join('')}</div>
    </div>
    <div style="margin-bottom:10px"><div class="fl" style="margin-bottom:6px"><span class="tag ta">Performance Tasks (50%)</span><span style="font-size:10px;color:var(--tx2)">Total: ${c.ptT!==null?c.ptT:'—'} · PS: ${c.ptPS!==null?c.ptPS:'—'}% · WS: ${c.ptWS!==null?c.ptWS:'—'}</span></div>
      <div style="display:flex;flex-wrap:wrap;gap:4px">${Array.from({length:ptC},(_,i)=>`<div style="text-align:center;padding:4px 3px;border:1px solid var(--bdr);border-radius:var(--rss);min-width:46px;background:var(--surf)"><div style="font-size:9px;color:var(--tx3)">PT${i+1}${h.pt[i]?'<br>/'+h.pt[i]:''}</div><div style="font-size:11px;font-weight:600;font-family:'DM Mono',monospace">${pt[i]!==null&&pt[i]!==undefined?pt[i]:'—'}</div></div>`).join('')}</div>
    </div>
    ${h.qa?`<div style="margin-bottom:10px"><div class="fl" style="margin-bottom:6px"><span class="tag tg">QA (20%)</span><span style="font-size:10px;color:var(--tx2)">Score: ${qa!==null?qa:'—'} / ${h.qa} · PS: ${c.qaPS!==null?c.qaPS:'—'}% · WS: ${c.qaWS!==null?c.qaWS:'—'}</span></div></div>`:''}
    <div style="padding:9px 12px;background:var(--surf2);border-radius:var(--r);border:1px solid var(--bdr);display:flex;gap:14px;flex-wrap:wrap">
      ${[['WW WS',c.wwWS],['PT WS',c.ptWS],['QA WS',c.qaWS],['Initial',c.initial]].map(([l,v])=>`<div><div style="font-size:9px;color:var(--tx3)">${l}</div><div style="font-size:11px;font-weight:600;font-family:'DM Mono',monospace">${v!==null?v:'—'}</div></div>`).join('')}
      <div><div style="font-size:9px;color:var(--tx3)">Q${q} Grade</div><div style="font-size:15px;font-weight:700;font-family:'DM Mono',monospace;color:${c.quarterly?(c.quarterly>=75?'var(--green)':'var(--red)'):'var(--tx3)'}">${c.quarterly||'—'}</div></div>
    </div>
  </div>`;
}
...
function pdfGSSD(){
  ...
  printWin('Student: '+name,`${hdr(escH(name)+' — Q'+q)}
    <table style="max-width:300px;margin-bottom:10px"><thead><tr>${allQ.map(a=>`<th>Q${a.q}</th>`).join('')}<th>Final</th></tr></thead>
    <tbody><tr>${allQ.map(a=>`<td class="bold ${a.c.quarterly?(a.c.quarterly>=75?'pass':'fail'):''}">${a.c.quarterly||'—'}</td>`).join('')}<td class="bold" style="color:#1a56db">${final||'—'}</td></tr></tbody></table>
    <table style="margin-bottom:8px"><thead><tr><th colspan="${wwC+3}" class="sh-ww">Written Works (30%)</th></tr><tr>${Array.from({length:wwC},(_,i)=>`<th>WW${i+1}</th>`).join('')}<th>Total</th><th>PS</th><th>WS</th></tr><tr class="hrow">${h.ww.slice(0,wwC).map(v=>`<td>${v||'—'}</td>`).join('')}<td>${h.ww.reduce((a,v)=>a+(v||0),0)}</td><td>100</td><td>30%</td></tr></thead><tbody><tr>${ww.slice(0,wwC).map(v=>`<td>${v!==null?v:''}</td>`).join('')}<td class="bold">${c.wwT!==null?c.wwT:''}</td><td>${c.wwPS!==null?c.wwPS:''}</td><td>${c.wwWS!==null?c.wwWS:''}</td></tr></tbody></table>
    <table style="margin-bottom:8px"><thead><tr><th colspan="${ptC+3}" class="sh-pt">Performance Tasks (50%)</th></tr><tr>${Array.from({length:ptC},(_,i)=>`<th>PT${i+1}</th>`).join('')}<th>Total</th><th>PS</th><th>WS</th></tr><tr class="hrow">${h.pt.slice(0,ptC).map(v=>`<td>${v||'—'}</td>`).join('')}<td>${h.pt.reduce((a,v)=>a+(v||0),0)}</td><td>100</td><td>50%</td></tr></thead><tbody><tr>${pt.slice(0,ptC).map(v=>`<td>${v!==null?v:''}</td>`).join('')}<td class="bold">${c.ptT!==null?c.ptT:''}</td><td>${c.ptPS!==null?c.ptPS:''}</td><td>${c.ptWS!==null?c.ptWS:''}</td></tr></tbody></table>
    ${h.qa?`<table style="max-width:250px;margin-bottom:8px"><thead><tr><th colspan="3" class="sh-qa">QA (20%)</th></tr><tr><th>Score</th><th>PS</th><th>WS</th></tr><tr class="hrow"><td>${h.qa}</td><td>100</td><td>20%</td></tr></thead><tbody><tr><td>${qa!==null?qa:''}</td><td>${c.qaPS!==null?c.qaPS:''}</td><td>${c.qaWS!==null?c.qaWS:''}</td></tr></tbody></table>`:''}
    <table style="max-width:340px"><thead><tr><th>WW WS</th><th>PT WS</th><th>QA WS</th><th>Initial</th><th>Q${q} Grade</th></tr></thead><tbody><tr><td>${c.wwWS!==null?c.wwWS:'—'}</td><td>${c.ptWS!==null?c.ptWS:'—'}</td><td>${c.qaWS!==null?c.qaWS:'—'}</td><td class="bold">${c.initial!==null?c.initial:'—'}</td><td class="bold ${c.quarterly?(c.quarterly>=75?'pass':'fail'):''}">${c.quarterly||'—'}</td></tr></tbody></table>`);
}
